...
<form method="post" onsubmit="return confirmInsert();">
    ...
    <input type="date" id="passenger_id" name="passengerid" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO user_activity(activity_id, UserId, activity_name,date,duration) VALUES (?, ?, ?, ? ,?)");
    $stmt->bind_param("issss", $activity_id, $UserId, $activity_name,$date,$duration);
    // Set parameters and execute
    $activity_id = $_POST['book_id'];
    $UserId = $_POST['ride_id'];
    $activity_name = $_POST['passenger_id'];
    $date= $_POST['passengerid'];
    $duration= $_POST['passengeid'];
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>user_activity Details</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>user_activity Table</h2></center>
    <table border="3">
        <tr>
            <th>activity_id</th>
            <th>UserId</th>
...
// Prepare SQL query to retrieve all user_activity
$sql = "SELECT * FROM user_activity";
$result = $connection->query($sql);

// Check if there are any user_activity
if ($result->num_rows > 0) {
    // Output data for each row
    while ($row = $result->fetch_assoc()) {
        $activity_id = $row['activity_id']; // Fetch the activity_id
        echo "<tr>
            ...
            <td>" . $row['date'] . "</td>
            <td>" . $row['duration'] . "</td>
            <td><a style='padding:4px' href='delete_user_activity.php?activity_id=$activity_id'>Delete</a></td> 
            <td><a style='padding:4px' href='update_user_activity.php?activity_id=$activity_id'>Update</a></td> 
        </tr>";
    }
