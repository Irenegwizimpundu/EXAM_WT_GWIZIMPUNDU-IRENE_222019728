...
<section>
    <h1><u>Exercises Form</u></h1>

<form method="post" onsubmit="return confirmInsert();">

    <label for="exercise_id">Exercise_id:</label>
    <input type="number" id="book_id" name="book_id" required><br><br>

    <label for="exercise_name">Exercise_name:</label>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO exercises(exercise_id, exercise_name, description) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("iss", $exercise_id, $exercise_name, $description);
    // Set parameters and execute
    $exercise_id = $_POST['book_id'];
    $exercise_name = $_POST['ride_id'];
...
<body>
    <center><h2>Exercises Table</h2></center>
    <table border="3">
        <tr>
            <th>exercise_id</th>
            <th>exercise_name</th>
...
// Prepare SQL query to retrieve all exercises
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $exercise_id = $row['exercise_id']; // Fetch the exercise_id
        echo "<tr>

            <td>" . $row['exercise_id'] . "</td>
            <td>" . $row['exercise_name'] . "</td>
            <td>" . $row['description'] . "</td>
            <td><a style='padding:4px' href='delete_exercises.php?exercise_id=$exercise_id'>Delete</a></td> 
            <td><a style='padding:4px' href='update_exercises.php?exercise_id=$exercise_id'>Update</a></td> 
        </tr>";
    }
