...
<section>
    <h1><u>Meal_food Form</u></h1>


<form method="post" onsubmit="return confirmInsert();">


    <label for="meal_food_id">Goal_id:</label>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO meal_food(meal_food_id,food_id, quantity) VALUES (?, ?, ?)");
    $stmt->bind_param("isS", $meal_food_id, $food_id,$quantity);
    // Set parameters and execute
    $meal_food_id = $_POST['book_id'];
    $food_id = $_POST['ride_id'];
    $quantity = $_POST['passenger_id'];
...
<body>
    <center><h2>meal_food Table</h2></center>
    <table border="3">
        <tr>
...
// Prepare SQL query to retrieve all meal_food
$sql = "SELECT * FROM meal_food";
$result = $connection->query($sql);

// Check if there are any meal_food
if ($result->num_rows > 0) {
    // Output data for each row
    while ($row = $result->fetch_assoc()) {
        $meal_food_id = $row['meal_food_id']; // Fetch the meal_food_id
        echo "<tr>

            <td>" . $row['meal_food_id'] . "</td>
            <td>" . $row['food_id'] . "</td>
            <td>" . $row['quantity'] . "</td>
            <td><a style='padding:4px' href='delete_meal_food.php?meal_food_id=$meal_food_id'>Delete</a></td> 
            <td><a style='padding:4px' href='update_meal_food.php?meal_food_id=$meal_food_id'>Update</a></td> 
        </tr>";
    }
