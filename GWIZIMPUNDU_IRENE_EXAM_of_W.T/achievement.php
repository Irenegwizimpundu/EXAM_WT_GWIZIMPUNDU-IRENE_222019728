...
<form method="post" onsubmit="return confirmInsert();">
    ...
    <label for="date_unlocked">date_unlocked:</label>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO achievement(achievement_id, UserId, achievement_name, date_unlocked) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("isss", $achievement_id, $UserID, $achievement_name, $date_unlocked);
    // Set parameters and execute
    $achievement_id = $_POST['book_id'];
    $UserID = $_POST['ride_id'];
    $achievement_name = $_POST['passenger_id'];
    $date_unlocked = $_POST['booking_time'];
...
<body>
    <center><h2>achievement Table</h2></center>
    <table border="3">
        <tr>
...
// Prepare SQL query to retrieve all achievement
$sql = "SELECT * FROM achievement";
$result = $connection->query($sql);

// Check if there are any achievement
if ($result->num_rows > 0) {
    // Output data for each row
    while ($row = $result->fetch_assoc()) {
        $achievement_id = $row['achievement_id']; // Fetch the achievement_id
        echo "<tr>

            <td>" . $row['achievement_id'] . "</td>
            <td>" . $row['UserId'] . "</td>
            <td>" . $row['achievement_name'] . "</td>
            <td>" . $row['date_unlocked'] . "</td>
            <td><a style='padding:4px' href='delete_achievement.php?achievement_id=$achievement_id'>Delete</a></td> 
            <td><a style='padding:4px' href='update_achievement.php?achievement_id=$achievement_id'>Update</a></td> 
        </tr>";
    }
